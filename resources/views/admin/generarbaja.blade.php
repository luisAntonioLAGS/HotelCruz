    ...
    <div class="xl:pl-72">
        ...
        <main>
            <div class="grid max-w-7xl grid-cols-1 gap-x-8 gap-y-10 px-4 py-16 sm:px-6 md:grid-cols-3 lg:px-8">
                <div>
                    <h2 class="text-base font-semibold leading-7 text-white">Baja Empleado</h2>
                ...
                <form class="flex items-start md:col-span-2" method="POST" action="{{route('generarbajapost')}}">
                    @csrf
                    <input id="claveempleado" name="claveempleado" type="hidden" value="{{ $claveempleado }}">
                    <button type="submit" class="rounded-md bg-red-500 px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-red-400">Si, deseo confirmar la baja</button>
                    @if(session('error'))
