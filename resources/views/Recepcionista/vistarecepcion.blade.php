    ...
    <main>
        <header>
            ...
            <div class="flex flex-col items-start justify-between gap-x-8 gap-y-4 bg-gray-700/10 px-4 py-4 sm:flex-row sm:items-center sm:px-6 lg:px-8">
                <a href="registrocliente" class="order-first flex-none rounded-full bg-indigo-600 px-2 py-2 text-md font-medium text-white ring-1 ring-inset ring-indigo-400/30 sm:order-none text">Crear
                    Reservación</a>
            </div>
            ...
            <div class="grid grid-cols-1 bg-gray-700/10 sm:grid-cols-2 lg:grid-cols-4">
                ...
                <div class="border-t border-white/5 py-6 px-4 sm:px-6 lg:px-8">
                    ...
                    <p class="mt-2 flex items-baseline gap-x-2">
                        <span class="text-4xl font-semibold tracking-tight text-white">{{$principal->HabitacionesDisponibles}}</span>
                    </p>
                </div>
                <div class="border-t border-white/5 py-6 px-4 sm:px-6 lg:px-8 sm:border-l">
                    ...
                    <p class="mt-2 flex items-baseline gap-x-2">
                        <span class="text-4xl font-semibold tracking-tight text-white">{{$principal->Reservaciones}}</span>
                    </p>
                </div>
                @endforeach
            ...
            <table class="mt-6 w-full whitespace-nowrap text-left">
                ...
                <thead class="border-b border-white/10 text-sm leading-6 text-white">
                    <tr>
                        ...
                        <th scope="col" class="hidden py-2 pl-0 pr-8 font-semibold sm:table-cell">Piso Habitación
                        </th>
                        <th scope="col" class="hidden py-2 pl-0 pr-8 font-semibold sm:table-cell">No Habitación
                        </th>
                        <th scope="col" class="hidden py-2 pl-0 pr-8 font-semibold sm:table-cell">Estatus
                        </th>
                        <th scope="col" class="py-2 pl-0 pr-4 text-right font-semibold sm:pr-8 sm:text-left lg:pr-20">
                            Fecha de entrada</th>
                        <th scope="col" class="hidden py-2 pl-0 pr-6 text-right font-semibold sm:table-cell sm:pr-6 lg:pr-6">
                            Fecha de Salida</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-white/5">
                    @foreach($infoDash as $reservacion)
                    <tr>
                        <td class="py-4 pl-4 pr-8 sm:pl-6 lg:pl-8">
                            <div class="flex items-center gap-x-4">
                                <div class="truncate text-sm font-medium leading-6 text-white">{{$reservacion->NombreCliente}}
                                </div>
                            </div>
                        </td>
                        <td class="hidden py-4 pl-0 pr-4 sm:table-cell sm:pr-8">
                            <div class="truncate text-sm font-medium leading-6 text-white">{{$reservacion->PisoHabitacion}}
                            </div>
                        </td>
                        <td class="hidden py-4 pl-0 pr-4 sm:table-cell sm:pr-8">
                            <div class="flex gap-x-3">
                                <div class="font-mono text-sm leading-6 text-gray-400">{{$reservacion->NoHabitacion}}</div>
                            </div>
                        </td>
                        <td class="hidden py-4 pl-0 pr-8 text-sm leading-6 text-gray-400 md:table-cell lg:pr-20">
                            <time datetime="2023-01-23T11:00">{{$reservacion->EstatusValidacion}}</time>
                        </td>
                        <td class="hidden py-4 pl-0 pr-8 text-sm leading-6 text-gray-400 md:table-cell lg:pr-20">
                            <time datetime="2023-01-23T11:00">{{$reservacion->FechaInicio}}</time>
                        </td>
                        <td class="hidden py-4 pl-0 pr-4 text-right text-sm leading-6 text-gray-400 sm:table-cell sm:pr-6 lg:pr-8">
                            <time datetime="2023-01-23T11:00">{{$reservacion->FechaFin}}</time>
                        </td>
                    </tr>
                    @endforeach
